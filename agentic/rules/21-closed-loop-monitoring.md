...
## Slicing contract (template-ADR-007)
...
## Ground-truth contract (template-ADR-006)
...
## Champion/Challenger contract (template-ADR-008)
...
- **Streaming ingestion** (Kafka/Bytewax): deferred by template-ADR-006 (revisit
  trigger: >100M predictions/day or sub-minute label latency requirements).
- **Full feature store** (Feast): out of scope per template-ADR-003.
- **Shadow mode with Istio traffic mirroring**: noted in template-ADR-008 as future
