...
2. **OpenAPI snapshot** (`tests/contract/openapi.snapshot.json` (written by this skill; it does not exist until then)) — committed
...
- **ALWAYS** re-run `services/demand-forecast-serving/scripts/refresh_contract.py` after any schema edit
...
- template-ADR-012 — API evolution policy: **withdrawn** (number retired); the policy
  ...
  `docs/decisions/template-ADR-012-api-evolution-policy.md` (an ml-service-template ADR; this repository's decisions are renumbered under `docs/decisions/`).
