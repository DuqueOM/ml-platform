...
| Q-06 | **One-sided vendored edit** — changing a file that exists in both the repo root and `templates/service/` (scripts, agentic surface, AGENTS.md) on only one side | `scripts/check_cicd_template_drift.py` (belongs to ml-service-template; its counterpart was REJECTED in `docs/governance/upstream-parity.yaml` because `services/` is vendored, so that drift is upstream's finding to make), `sync_agentic_adapters.py --check`, vendored-runtime drift gates | Apply to both sides in the same commit |
