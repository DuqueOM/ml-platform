...
authorization_mode:
  ...
  interpret_thresholds: AUTO     # apply template-ADR-008 rules to numbers
...
you need ground truth (template-ADR-006). The sliced performance monitor does this:
...
(template-ADR-008). Do NOT expect retraining to automatically replace the champion —
...
- [ ] If labels available: sliced AUC/F1 report computed for the configured
      slices in `configs/slices.yaml` (template-ADR-007)
...
- template-ADR-006 — Closed-loop monitoring
- template-ADR-007 — Sliced performance analysis
- template-ADR-008 — Champion/challenger gate
- template-ADR-009 — Retraining triggers
