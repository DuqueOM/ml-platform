---
name: edge-audit
description: Scan a service for edge-protection coverage (WAF, DDoS mitigation, rate limiting) against template-ADR-042/D-38 — produces a PASS/FAIL report with file:line evidence
allowed-tools:
  - Read
  - Grep
  - Glob
...
# Edge Audit — Coverage scan against template-ADR-042 / D-38
...
- **To actually add edge protection** — this skill audits; `/edge-setup`
  ...
  fix.
- **On a `local` stack profile** — the local-first profile (template-ADR-033,
...
verbs (template-ADR-042 §2.2), the same separation `ci-green-verify` draws
...
- **Not a live traffic/WAF-hit analyzer** — reading actual blocked/
  ...
  of template-ADR-042), not this skill's.
