...
| **GV.RR** Roles, responsibilities, authorities | Roles and authorities are established, communicated and enforced | Partial | `.github/CODEOWNERS` maps every path to an owner with the reasoning inline; `AGENTS.md` carries an AUTO/CONSULT/STOP authority matrix covering roughly forty operations, and `scripts/validate_agentic_surface.py --strict` fails CI if a rendered tool surface de-escalates a mode. **Missing, and the artifact says so itself**: CODEOWNERS states "@DuqueOM is the only maintainer… this file distributes no review today", and branch protection requires `required_approving_review_count: 0`. There is no separation of duties, and one maintainer cannot create one — a person cannot approve their own pull request. What protection now delivers is the MECHANISM without the CONTROL: pull requests are mandatory, all four CI jobs must pass, `enforce_admins` applies the rule to the owner, and every merge leaves a record. An adopter with two engineers turns this row from Partial to Satisfied by raising one number; this repository cannot |
...
| Category | What it asks for | Verdict | Evidence, or the reason there is none |
| --- | --- | :-: | --- |
| **PR.AA** Identity, authentication, access control | Identities are managed and access is granted on least privilege | Partial | Repository side: `.github/CODEOWNERS`, and `scripts/setup_branch_protection.sh` as branch protection expressed in code rather than clicked. Cloud side: every overlay pairs an ExternalSecret with a SecretStore, and `tests/test_gitops_manifests.py` asserts per cloud that the auth block contains `workloadIdentity` or `jwt` and contains no `secretRef` — federated identity, no stored key, checked rather than promised. **Corrected this round**: the payload required only two of the four CI jobs, and — worse — the script had never been run, so `main` had no protection of any kind. An audit read the script as the configuration; the API said "Branch not protected". All four jobs are required now and the protection is applied, which is the difference between a control expressed in code and a control. The approving review count remains zero for the reason under GV.RR. No identity has ever been exercised against a cloud (L4 = 0), so least privilege is a property of the declaration only |
...
| **PR.PS** Platform security | Hardware, software and services are managed consistent with risk | Partial | `platform/kubernetes/base/deployment.yaml` sets `runAsNonRoot`, `runAsUser: 10001`, `allowPrivilegeEscalation: false`, `readOnlyRootFilesystem: true` and drops all capabilities. `platform/policies/` ships a default-deny NetworkPolicy with explicit allow-DNS and allow-ingress companions, included by all six cloud overlays, and `tests/test_gitops_manifests.py` asserts per overlay that default-deny and allow-dns are both present — the second because a default-deny namespace with no DNS egress breaks every lookup. Every overlay is rendered in CI. **Missing**: no workflow builds, pushes, signs or attests any image, so the admission control that would make a signature mandatory has nothing to rule on and does not exist. **Corrected by QA-4 round eleven**, which found this row still reporting three gaps that had closed: every overlay — all six cloud cells and `local` — now ships a Namespace at `pod-security.kubernetes.io/enforce: restricted`, asserted by `tests/test_gitops_manifests.py::test_every_overlay_creates_its_namespace_at_restricted`; the base image is an unresolvable placeholder tag rather than `:latest`, asserted by `test_no_overlay_ships_a_mutable_image_tag`; and `platform/policies/README.md` describes the NetworkPolicies it holds rather than Kyverno policies it never did. A fourth policy, `allow-serving-egress`, now scopes egress to the metadata server on port 80, public HTTPS with the private ranges excluded, and the OTLP collector. **Closed since**: those `namespaceSelector` peers selected labels no namespace carried, so the pod could be neither scraped nor reached under `default-deny`. `platform/policies/namespace-contract.yaml` now declares each namespace role and its provisioner, the policies select those roles, and a test fails on a selector nobody declared. What remains is evidence rather than design: no cluster has yet been observed enforcing these policies |
| **PR.IR** Technology infrastructure resilience | Infrastructure is managed to achieve resilience requirements | Partial | A PodDisruptionBudget in the base; production replicas asserted greater than dev per cloud; an ArgoCD ApplicationSet with production auto-sync disabled and pruning disabled everywhere, both asserted by tests, so neither a bad commit nor a rendering mistake removes running workloads by itself. **Missing**: no resilience requirement is stated anywhere to be met — no availability target, no capacity plan, no failover design. Nothing has ever run. The local cluster CAN close part of the gap for the network controls, which this row denied until QA-4 round eleven: on a throwaway kind v0.30 cluster, kindnetd enforced a default-deny and the rendered policies timed DNS out, so kind is capable of the enforcement evidence this row said it could not produce. The local overlay does not yet apply the policies, and the test still asserts only that a kindnet daemonset exists — both recorded in `docs/governance/remediation-work-order.md`, round eleven |
...
**Both findings in this place have closed, and the text is kept as the record
of what they were.** The base Deployment's comment said Pod Security
"restricted" was the namespace default while only `local` shipped a Namespace,
so the six cloud overlays carried no enforcement label; and the image was
pinned to `:latest`, a mutable tag, in a repository that argues for immutable
references everywhere else. Every overlay now ships a Namespace at `restricted`
and the base image is a placeholder tag that cannot resolve, each asserted by a
test in `tests/test_gitops_manifests.py`. This section went on reporting both
as open until QA-4 round eleven — a compliance document is the last place a
closed gap should still read as open, because it is the one an adopting
organisation reads to decide what it inherits.
