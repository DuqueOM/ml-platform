...
| P2 | Type-checked | `uv run mypy libs/ scripts/ projects/demand-forecast/src/ projects/rag-assistant/src/ projects/store-assistant/src/` | Zero errors, `strict` over everything in scope | This row said "strict on `libs/`" and was wrong for the repository's whole history: `strict` sat in a per-module override, and mypy applies that flag globally while reporting the module list as unused. Measured, not read — the narrower claim understated what already ran, and `tests/test_type_gate_enforces_its_config.py` now runs known-bad code through this config so the threshold is watched failing |
