...
| P8 ⚠️ | Kubernetes posture | `kubescape scan framework nsa,cis-v1.10.0` on rendered overlays | **Advisory — reports, does not block** | Same defect as P7, same correction: the step carries `continue-on-error: true`. What the overlays DECLARE is separately gated — `tests/test_gitops_manifests.py` fails on a namespace without restricted Pod Security labels, and that one blocks. The **tool** is no longer advisory, only the findings: the pinned, checksum-verified download is its own step and fails the job, after the previous form spent weeks fetching a 404 |
...
| P10 | Third-party actions identify a program | `uv run python scripts/check_action_pins.py` | Every `uses:` a 40-char commit SHA with its tag in a comment, and no workflow downloads from a moving URL | A tag is a mutable pointer to code that runs with the job's token; re-pointing it swaps the program with no commit here, and a subverted scanner reports nothing — the same output as a clean tree. **Widened after it missed the case that bit**: the gate read `uses:` only, while CI downloaded kubescape from `releases/latest/download` in a `run:` block. The asset was renamed upstream, the request began 404ing, and the step's `continue-on-error` reported that as success — a scanner that never ran produces no findings, which is indistinguishable from a clean tree (QA-4 W-8) |
| P11 | Shared libraries are reused, not decorated | `uv run python scripts/check_library_reuse.py` | Every declared library imported, every imported library declared, no library symbol reimplemented, and no project below its recorded reuse floor | Charter C1 counts libraries a project reuses, so a declaration nobody imports raises the count without reusing anything — found twice in demand-forecast on the gate's first run. The floor makes the plan's Phase 4 precondition enforceable as a ratchet: "has it dropped back" is answerable on every commit where "has it got there yet" is not |
...
| P13 | Third-party data is pinned | `uv run pytest tests/test_dataset_lock.py -q` | Every dataset fetchable over HTTPS is pinned by sha256 or carries a written reason it is not | `scripts/datasets/fetch.py` hashed every download from the day it was written — into a manifest under the gitignored `data/`, where no reviewer or CI job could read it. The digests proved one machine kept getting the same bytes, never that everybody does. This row gates the lock's coherence against the registry, which is what CI can prove without the data; the BYTE check is `uv run python scripts/datasets/fetch.py --verify`, which needs the data and therefore runs where the data is (ADR-009). Stated rather than papered over: a test that downloaded 263 MB per commit would buy little, because a source changing under a stable URL already surfaces as a diff here |
| P14 | The model artifact survives the serving seam | `uv run python scripts/check_artifact_compatibility.py` | Every version `uv.lock` resolves for numpy, scikit-learn and joblib satisfies the specifier `services/demand-forecast-serving/requirements.txt` installs | The workspace pickles a model and the container unpickles it, and a joblib load across an incompatible numpy does not raise — it returns arrays it has misread, and the service serves predictions from them. There is no traceback and no error rate; the first evidence is a metric moving for a reason nobody can attribute. Both sides already knew: the container's own requirements file carries a comment warning that numpy 2.x silently corrupts joblib models, and nothing compared them. **Three straddles exist today** and are exempted BY NAME against [ADR-008](../decisions/ADR-008-serving-a-forecast-from-a-classification-scaffold.md), whose interface half needs a human decision — so this gate is green on the known and red when that decision lands. **It cannot go red on a new straddle**, which this row first promised: all three compared packages are exempt, and a straddle in any other package is not compared — QA-4 round eleven added two and got OK. Nor does it check that the container can import the modules the pickle references, and today it cannot: the artifact pickles workspace-library types the image does not install, recorded against ADR-008. The exemption lifts mechanically when that ADR stops declaring itself Proposed, and an exemption whose straddle has been fixed also fails, because a list that keeps naming findings nobody has is where findings go to die |
| P15 | The project generator still renders | `uv run python scripts/check_template_render_safety.py` | Every file and every path segment under `templates/project/` parses as a Copier template | `copier.yml` sets `_templates_suffix: ""`, so EVERY payload file is a Jinja template and a stray delimiter does not render oddly — it aborts `copier copy`, and whoever ran the generator gets nothing. [tests/test_project_generator.py](../../tests/test_project_generator.py) renders the payload for real and remains the authority on behaviour. **This row first said that test rendered one answer set; it rendered three of the four kinds**, omitted only the agent kind, and now renders all four — QA-4 round eleven caught the false claim. What parsing adds is not coverage of forgotten kinds: a render exercises only the branches its answers select, while a parse examines every branch of every file for any answers, including values nobody enumerates, and costs milliseconds rather than a render. A render root holding no payload file is a setup error, not a pass. The threshold includes path SEGMENTS because copier renders those too and this payload has one, `src/{@ project_slug @}/` — a class the upstream original does not check. Watched failing on each defect class in [tests/test_template_render_safety.py](../../tests/test_template_render_safety.py), against a temporary tree rather than by breaking the payload three other checks read |
