...
| P15 | The project generator still renders | `uv run python scripts/check_template_render_safety.py` | Every file and every path segment under `templates/project/` parses as a Copier template | `copier.yml` sets `_templates_suffix: ""`, so EVERY payload file is a Jinja template and a stray delimiter does not render oddly — it aborts `copier copy`, and whoever ran the generator gets nothing. [tests/test_project_generator.py](../../tests/test_project_generator.py) renders the payload for real and remains the authority on behaviour, but it renders **one answer set**, the tabular kind, while `copier.yml` offers four kinds; a file that breaks only under the LLM or agent kind passes the entire suite and fails in the adopter's terminal. Parsing is answer-independent, so it covers all four, and it costs milliseconds rather than a render. The threshold includes path SEGMENTS because copier renders those too and this payload has one, `src/{@ project_slug @}/` — a class the upstream original does not check. Watched failing on each defect class in [tests/test_template_render_safety.py](../../tests/test_template_render_safety.py), against a temporary tree rather than by breaking the payload three other checks read |
