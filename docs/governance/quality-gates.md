...
| P16 | The contract's deviations are derived from the data | `uv run python scripts/check_contract_deviations.py --check` | Every entry in KNOWN_DEVIATIONS appears in the contract's table, with its reason | [docs/PROJECT_CONTRACT.md](../PROJECT_CONTRACT.md) says the test is the authority and then narrated a single deviation while the dictionary held four, across two projects — in the document a reader consults to decide whether their own project complies. Prose that restates data drifts from it, and an exemption list is where that drift does the most damage: the reader believes the short version. The table is generated now, so the two cannot separate, and the dictionary stays in the test because that is what makes an exemption self-cleaning — an entry for a requirement a project has come to satisfy fails the suite |
