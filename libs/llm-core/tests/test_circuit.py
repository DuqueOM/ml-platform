...
from llm_core.circuit import CircuitBreaker, State
...
def test_effective_tier_degrades() -> None:
...
def test_effective_tier_none_when_all_open() -> None:
    cb = CircuitBreaker(failure_threshold=1)
    for tier in (0, 1, 2):
...
def test_tiers_are_independent() -> None:
