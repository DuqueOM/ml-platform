...
from llm_core.retrieval import BM25Index, make_semantic_retrieval

_MARKER = " …[truncated]"


def _corpus(tmp_path: Path, target_body: str) -> Any:
...
def test_search_truncates_to_max_chars(tmp_path: Path) -> None:
...
def test_search_without_cap_returns_full(tmp_path: Path) -> None:
...
def test_semantic_retrieval_tool_applies_cap(tmp_path: Path) -> None:
