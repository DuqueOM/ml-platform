...
import pytest
from llm_core.schemas import Observation, RequestBudget, Route, Verdict
...
def test_route_invalid_finality() -> None:
    ...
    with pytest.raises(ValidationError):  # ValidationError
        Route(
            ...
            ambiguity="low",
            tool_needed=False,
            # The invalid literal IS the subject: the test asserts pydantic
            # rejects it at runtime, which the type checker cannot express.
            finality="not_a_finality",  # type: ignore[arg-type]
            expected_followup=False,
        )


def test_route_confidence_bounds() -> None:
...
def test_request_budget_defaults() -> None:
...
def test_verdict_approved() -> None:
...
def test_verdict_rejected() -> None:
