...
def _http_error(status: int, retry_after: Any | None = None) -> httpx.HTTPStatusError:
...
def test_with_retry_does_not_retry_terminal() -> None:
    calls = {"n": 0}
...
def test_with_retry_honors_retry_after() -> None:
    slept: list[float] = []
    calls = {"n": 0}
...
def test_tier_client_retries_then_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    attempts = {"n": 0}

    def fake_post(url: Any, json: Any, headers: Any, timeout: Any) -> Any:
        attempts["n"] += 1
        if attempts["n"] < 2:
    ...
    monkeypatch.setattr("llm_core.tiers.httpx.post", fake_post)
...
def test_retry_policy_from_config() -> None:
...
def test_bare_url_string_is_a_local_endpoint() -> None:
...
def test_remote_endpoint_requires_a_model() -> None:
...
def test_remote_endpoint_reads_credential_from_named_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_missing_credential_raises_rather_than_calling_unauthenticated(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_blank_credential_is_treated_as_missing(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_remote_endpoints_do_not_support_grammar() -> None:
...
def test_adapt_constraints_leaves_local_payload_untouched() -> None:
...
def test_adapt_constraints_rewrites_schema_for_remote() -> None:
    payload = {"messages": [], "grammar": "root ::= x", "json_schema": {"type": "object"}}
...
def test_resolve_collapses_to_the_highest_configured_tier_below() -> None:
...
def test_resolve_prefers_an_exact_tier_when_configured() -> None:
...
def test_resolve_raises_when_no_tier_is_configured_below() -> None:
    client = TierClient({2: "http://127.0.0.1:8093/v1"})
    with pytest.raises(KeyError, match="no tier configured"):
        client.resolve(1)


def test_call_sends_auth_header_and_model_for_remote_tier(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_call_degrades_to_local_tier_when_higher_tier_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    seen: dict[str, Any] = {}
