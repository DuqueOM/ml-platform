# store-ADR-007: Structured tool-calling contract
...
- **Related**: store-ADR-006 (tool capability contract), the Tier-0 routing grammar
...
validated defensively with the per-tool Pydantic `args_model` (I-5, store-ADR-006).
...
2. **Schema generated from the registry, not hand-written.** A static grammar
   ...
   burden: tools already declare an optional `args_model` (store-ADR-006).
