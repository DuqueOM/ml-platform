# store-ADR-010 — MCP / A2A Interoperability: Rejected (with revisit triggers)
...
capability gate (store-ADR-006) → argument validation → execution → policy gate
...
### As a client: rejected — the annotation trust model conflicts with store-ADR-006
...
store-ADR-006's fail-closed contract requires a tool's `read_only`/`dry_run_only`
...
honoring store-ADR-006, every consumed MCP tool would need a manually-audited,
...
- store-ADR-006's fail-closed guarantee stays enforceable exactly as designed —
...
- `A2A` (multi-agent coordination) was not separately evaluated in depth:
  this platform is single-agent-with-usecases (store-ADR-001), not
...
- MCP's specification promotes tool-capability annotations from "hint,
  ...
  2026-03) — this would remove the store-ADR-006 conflict this ADR is built on.
...
- `docs/decisions/store-ADR-006-tool-capability-contract.md` — the fail-closed
...
- `ml-service-template` `docs/decisions/ADR-029-agentic-adoption-contract.md` —
