# store-ADR-011 — Hybrid tier topology: resident memory is the binding constraint
...
- OpenAI-compatible is a family, not a standard. A provider that rejects
  ...
  (store-ADR-007's existing escape hatch), which drops the planner back to the
...
| Collapse to a single tier permanently | Discards cross-tier verification (store-ADR-004), which is the platform's strongest safety property — the verifier must run *above* the generator, never self-review |
...
- `docs/decisions/store-ADR-002-calibrated-infrastructure.md` — the same
...
- `docs/decisions/store-ADR-004-cross-tier-verification.md` — why collapsing to one
  tier is not an acceptable simplification.
- `docs/decisions/store-ADR-007-structured-tool-calling.md` — the
  `structured_tool_calls: false` escape hatch this ADR reuses for providers
  lacking schema-constrained output.
- `docs/decisions/store-ADR-008-retrieval-caller-isolation.md` — the caller-isolation
