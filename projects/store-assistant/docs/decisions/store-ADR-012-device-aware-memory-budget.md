# store-ADR-012 — "Local" is two budgets, not one: device-aware memory invariant

- **Status**: Accepted
- **Date**: 2026-08-05
- **Amends**: [store-ADR-011](store-ADR-011-hybrid-tier-topology.md). The topology decision
  stands; the invariant it introduced was the wrong shape.

## Context

store-ADR-011 introduced `limits.max_local_tiers` to prevent a resident-memory
...
**But store-ADR-011's invariant cannot express any of this.** It counts local tiers
...
| Swap Tier 0 for a larger GPU-resident model, route remotely | Loses GBNF grammar constraints on routing — the exact weakening store-ADR-011 identifies as disqualifying `all-remote` from the quality-gated path |
...
- [store-ADR-011](store-ADR-011-hybrid-tier-topology.md) — the topology this amends.
- [store-ADR-002](store-ADR-002-calibrated-infrastructure.md) — the same principle:
