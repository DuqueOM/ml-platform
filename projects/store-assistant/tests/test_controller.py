...
from llm_core.circuit import State
from llm_core.controller import RunContext, _coerce, _split_args
from llm_core.schemas import RequestBudget, Route
...
class FakeTiers(TierResolutionStub):
    ...
    def __init__(self, contents: Any | None = None, fail: Any = False) -> None:
...
def _fixed_route(intent: Any = "smalltalk", tier: Any = 0, confidence: Any = 0.98, risk: Any = "low") -> Any:
    return Route(
        intent=intent,
        tier=tier,
...
def test_happy_path(agent: Any) -> None:
...
def test_tier_failure_degrades_to_safe_template(agent: Any) -> None:
...
def test_open_circuit_skips_tier_calls(agent: Any) -> None:
...
def test_breaker_records_failure_on_single_request(agent: Any) -> None:
    breaker = agent.controller.breaker
    agent.tiers = FakeTiers(fail=True)
    agent.handle("hola")
...
def test_latency_budget_degrades_to_safe_template(agent: Any, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_extract_tool_calls_parses_multiple_args(agent: Any) -> None:
...
def test_extract_structured_single(agent: Any) -> None:
    ctx = _ctx(agent)
...
def test_extract_structured_multiple(agent: Any) -> None:
    ctx = _ctx(agent)
    content = (
        '{"tool_calls": ['
...
def test_extract_structured_empty(agent: Any) -> None:
    ctx = _ctx(agent)
    assert ctx.extract_tool_calls(_reply('{"tool_calls": []}')) == []


def test_extract_structured_drops_unknown_tool(agent: Any) -> None:
    ctx = _ctx(agent)
...
def test_extract_structured_tolerates_code_fence(agent: Any) -> None:
    ctx = _ctx(agent)
...
def test_extract_falls_back_to_legacy_text(agent: Any) -> None:
...
def test_plan_passes_json_schema_when_enabled(agent: Any) -> None:
...
def test_plan_omits_json_schema_when_disabled(agent: Any) -> None:
    class RecordingTiers(TierResolutionStub):
        def __init__(self):
            self.last_kwargs: dict | None = None
...
def test_reflection_note_reaches_generator_context(agent: Any, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_reflection_is_not_verifier_evidence(agent: Any, monkeypatch: pytest.MonkeyPatch) -> None:
