...
def _route(intent="product_lookup", tier=1, risk="low"):
    return Route(
        intent=intent,
        tier=tier,
        confidence=0.95,
...
def test_fooled_model_price_claim_without_lookup_is_blocked(agent):
...
def test_fooled_model_illegal_promise_is_blocked(agent):
...
def test_fooled_model_fabricated_confirmation_is_blocked(agent):
...
def test_fooled_model_promo_claim_is_blocked(agent):
...
def test_order_create_stays_dry_run_even_if_model_echoes_a_paid_flag(agent):
...
def test_unknown_tool_name_from_injected_plan_is_rejected(agent):
