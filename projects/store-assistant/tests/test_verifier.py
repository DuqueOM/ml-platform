...
class FakeTiers(TierResolutionStub):
    def __init__(
        self,
        *,
        generate: Any = "respuesta",
        critic: Any = "APPROVED",
        critic_votes: Any | None = None,
...
def _route(risk: Any, tier: Any = 1) -> Any:
    return Route(
        intent="complaint",
        tier=tier,
        confidence=0.95,
...
def _with_route(agent: Any, risk: Any, tier: Any = 1) -> None:
    agent.router.route = lambda msg: _route(risk, tier)  # type: ignore[assignment]


def test_low_risk_skips_verifier(agent: Any) -> None:
...
def test_verifier_runs_at_higher_tier(agent: Any) -> None:
...
def test_verifier_rejection_escalates_and_regenerates(agent: Any) -> None:
    _with_route(agent, "medium", tier=1)
...
def test_self_consistency_majority_approves(agent: Any) -> None:
...
def test_self_consistency_minority_rejects(agent: Any) -> None:
    agent.config.verification["self_consistency_k"] = 3
    _with_route(agent, "high", tier=1)
...
def test_self_consistency_only_for_high_risk(agent: Any) -> None:
...
def test_verification_can_be_disabled(agent: Any) -> None:
